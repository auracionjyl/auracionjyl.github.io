function Board() {
	...
	this.puzzle_white_position = restore_array("000000000000000000000000000000000000");
	this.game_status = "set puzzle";
	this.move_index = 0;
	this.last_move = 99;
	var that = this;
	...
	this.is_used = function (loc){
		if (this.black_position[loc] === 1){return true;}
		if (this.white_position[loc] === 1){return true;}
		return false;
	}
	...
	this.reset_puzzle = function (){
		that.tile_ids = [];
		for (var i = 0; i <= M*N; i++) {that.tile_ids.push(i);};
		that.tiles = [];
		that.black_position = restore_array("000000000000000000000000000000000000");
		that.white_position = restore_array("000000000000000000000000000000000000");
		that.canvas.empty();
		that.create_tiles();
		for(var i = 0; i <= M*N; i++){
			if(that.puzzle_black_position[i] === 1){
				that.add_piece(i, 0);
			}
			if(that.puzzle_white_position[i] === 1){
				that.add_piece(i, 1);
			}
		}
		that.game_status = "playing";
	}

	this.fraze_all = function(){
		$('.tile').off('mouseenter').off('mouseleave').off('click')
		$('.usedTile').off('mouseenter').off('mouseleave').off('click')
	}
	...
	this.add_piece = function(loc, col) {
		if(col == 0) {
			...
			if(that.game_status == "set puzzle"){
				that.puzzle_black_position[loc] = 1;
			}
		} else {
			...
			if(that.game_status == "set puzzle"){
				that.puzzle_white_position[loc] = 1;
			}
		}
	}
	...
	this.remove_sol_piece = function (loc){
		that.tiles[loc].empty();
	}
	
	this.remove_piece = function(loc){
		that.tiles[loc].empty().removeClass("usedTile").addClass("tile").on('mouseenter').on('mouseleave').css("backgroundColor", square_bkgcolor);
		that.highlight_tiles();
		that.black_position[loc]=0
		that.white_position[loc]=0
		if(that.game_status == "set puzzle"){
			that.puzzle_black_position[loc] = 0;
			that.puzzle_white_position[loc] = 0;
		}
	}
...
function Solver(){
	...
	this.select_branch = function (p){
		this.cur_state.push(String(p))
		return this.solutions[this.cur_state.join(",")]
	}

	this.init_branch = function(){
